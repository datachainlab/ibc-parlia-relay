...
func (r *mockChain) Header(_ context.Context, height uint64) (*types2.Header, error) {
	...
	if header.Number.Uint64()%constant.BlocksPerEpoch == 0 {
		...
		} else if header.Number.Int64() == 400 {
			header.Extra = append(header.Extra, make([]byte, extraVanity)...)
			for i := 1; i <= 5; i++ {
				header.Extra = append(header.Extra, common.Hex2Bytes(fmt.Sprintf("300000000000000000000000000000000000000%d", i))...)
			}
			header.Extra = append(header.Extra, make([]byte, extraSeal)...)
		} else {
		...
		if header.Number.Uint64() < 203 {
			header.Coinbase = common.BytesToAddress(common.Hex2Bytes(fmt.Sprintf("100000000000000000000000000000000000000%d", 1+header.Number.Uint64()%5)))
		} else if header.Number.Uint64() >= 411 && header.Number.Uint64() < 603 {
			header.Coinbase = common.BytesToAddress(common.Hex2Bytes(fmt.Sprintf("300000000000000000000000000000000000000%d", 1+header.Number.Uint64()%5)))
		} else {
			mod := 1 + header.Number.Uint64()%21
			if mod >= 10 {
				header.Coinbase = common.BytesToAddress(common.Hex2Bytes(fmt.Sprintf("20000000000000000000000000000000000000%d", mod)))
			} else {
				header.Coinbase = common.BytesToAddress(common.Hex2Bytes(fmt.Sprintf("200000000000000000000000000000000000000%d", mod)))
			}
		}
	}
	return header, nil
...
func (ts *ProverTestSuite) TestQueryLatestFinalizedHeader() {
	...
	for latest := uint64(2); latest < 1200; latest++ {
		...
		ts.Require().NoError(err)
		finalized := header.GetHeight().GetRevisionHeight()

		currentEpochBlock, err := ts.chain.Header(context.TODO(), getCurrentEpoch(finalized))
		ts.Require().NoError(err)
		currentValidator, err := extractValidatorSet(currentEpochBlock)
		ts.Require().NoError(err)
		previousEpochBlock, err := ts.chain.Header(context.TODO(), getPreviousEpoch(finalized))
		ts.Require().NoError(err)
		previousValidator, err := extractValidatorSet(previousEpochBlock)
		ts.Require().NoError(err)

		// assert validators
		downcast := header.(*Header)
		ts.Require().Equal(previousValidator, downcast.PreviousValidators)
		if finalized%constant.BlocksPerEpoch == 0 {
			ts.Require().Nil(downcast.CurrentValidators)
		} else {
			ts.Require().Equal(currentValidator, downcast.CurrentValidators)
		}

		// assert height
		ts.Require().Less(finalized, latest)
		checkpoint := currentEpochBlock.Number.Uint64() + checkpointHeight(len(previousValidator))
		finalizeToCurrent := ts.prover.requiredHeaderCountToFinalize(len(currentValidator))
		if latest >= checkpoint+(finalizeToCurrent-1) {
			// only finalized by current validator
			ts.Require().Equal(int64(latest-(finalizeToCurrent-1)), int64(finalized))
		} else {
			ts.Require().True(finalized == lastFinalized || finalized-1 == lastFinalized)
		}
...
func (ts *ProverTestSuite) TestCreateMsgCreateClient() {
	...
	assertFn := func(finalizedHeight int64) {
		...
		ts.Require().Equal(cs.TrustLevel.Denominator, uint64(2))
