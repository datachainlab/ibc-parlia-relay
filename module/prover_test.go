...
func (ts *ProverTestSuite) TestSuccess_setupHeadersForUpdate_neighboringEpoch() {
	...
	verify := func(latestHeight, nextHeight uint64, expected int) {
		...
		targets, err := setupHeadersForUpdate(fn, headerFn, nil, clientStateLatestHeight, latestFinalizedHeader)
		ts.Require().NoError(err)
...
func (ts *ProverTestSuite) TestSuccess_setupHeadersForUpdate_nonNeighboringEpoch() {

	verify := func(latestHeight, nextHeight uint64, expectedEpochs []int64, nonNeighboringEpochFinalized bool, additionalNotFinalized bool) {
		clientStateLatestHeight := clienttypes.NewHeight(0, latestHeight)
		h1, err := newETHHeader(&types2.Header{
			Number: big.NewInt(int64(nextHeight)),
		})
		ts.Require().NoError(err)
		h2, err := newETHHeader(&types2.Header{
			Number: big.NewInt(int64(nextHeight + 1)),
		})
		ts.Require().NoError(err)
		h3, err := newETHHeader(&types2.Header{
			Number: big.NewInt(int64(nextHeight + 2)),
		})
		ts.Require().NoError(err)
		h4, err := newETHHeader(&types2.Header{
			Number: big.NewInt(int64(nextHeight + 3)),
		})
		ts.Require().NoError(err)
		headers := []*ETHHeader{h1}
		if nonNeighboringEpochFinalized {
			headers = append(headers, h2, h3, h4) // append after checkpoint
		}
		latestFinalizedHeader := &Header{
			Headers:            headers,
		...
		f := false
		notFinalized := &t
		queryFn := func(height uint64, _ uint64) (core.Header, error) {
			if *notFinalized {
				notFinalized = &f
				return nil, nil
			} else {
		...
		queryAdditionalFn := func(checkpoint uint64, nextCheckpointMinusOne uint64) ([]*ETHHeader, error) {
			if additionalNotFinalized {
				return nil, nil
			} else {
				...
				h2, e := newETHHeader(&types2.Header{
					Number: big.NewInt(int64(checkpoint + 1)),
				})
				ts.Require().NoError(e)
				h3, e := newETHHeader(&types2.Header{
					Number: big.NewInt(int64(checkpoint + 2)),
				})
				return []*ETHHeader{h1, h2, h3}, e
			}
		}
		headerFn := func(_ context.Context, height uint64) (*types2.Header, error) {
			return &types2.Header{
		...
		targets, err := setupHeadersForUpdate(queryFn, headerFn, queryAdditionalFn, clientStateLatestHeight, latestFinalizedHeader)
		ts.Require().NoError(err)
	...
	verify(0, constant.BlocksPerEpoch-1, []int64{int64(constant.BlocksPerEpoch - 1)}, false, false)
	verify(0, constant.BlocksPerEpoch, []int64{int64(constant.BlocksPerEpoch)}, false, false)
	verify(0, constant.BlocksPerEpoch+1, []int64{}, false, false)
	verify(0, 10*constant.BlocksPerEpoch-1, []int64{400, 800, 1200, 1600}, false, false)
	verify(0, 10*constant.BlocksPerEpoch, []int64{400, 800, 1200, 1600, 2000}, false, false)
	verify(0, 10*constant.BlocksPerEpoch, []int64{400, 800, 1200, 1600, 2000}, true, false)
	verify(0, 10*constant.BlocksPerEpoch, []int64{400, 800, 1200, 1600}, false, true)
	verify(0, 10*constant.BlocksPerEpoch+1, []int64{400, 800, 1200, 1600, 2000, 2001}, false, false)
	verify(constant.BlocksPerEpoch-1, constant.BlocksPerEpoch-1, []int64{}, false, false)
	verify(constant.BlocksPerEpoch-1, constant.BlocksPerEpoch, []int64{int64(constant.BlocksPerEpoch)}, false, false)
	verify(constant.BlocksPerEpoch-1, constant.BlocksPerEpoch+1, []int64{}, false, false)
	verify(constant.BlocksPerEpoch-1, 10*constant.BlocksPerEpoch-1, []int64{400, 800, 1200, 1600}, false, false)
	verify(constant.BlocksPerEpoch-1, 10*constant.BlocksPerEpoch, []int64{400, 800, 1200, 1600, 2000}, false, false)
	verify(constant.BlocksPerEpoch-1, 10*constant.BlocksPerEpoch, []int64{400, 800, 1200, 1600, 2000}, true, false)
	verify(constant.BlocksPerEpoch-1, 10*constant.BlocksPerEpoch, []int64{400, 800, 1200, 1600}, false, true)
	verify(constant.BlocksPerEpoch-1, 10*constant.BlocksPerEpoch+1, []int64{400, 800, 1200, 1600, 2000, 2001}, false, false)
	verify(constant.BlocksPerEpoch, constant.BlocksPerEpoch, []int64{}, false, false)
	verify(constant.BlocksPerEpoch, constant.BlocksPerEpoch+1, []int64{int64(constant.BlocksPerEpoch + 1)}, false, false)
	verify(constant.BlocksPerEpoch, 10*constant.BlocksPerEpoch-1, []int64{600, 1000, 1400, 1800, 1999}, false, false)
	verify(constant.BlocksPerEpoch, 10*constant.BlocksPerEpoch, []int64{600, 1000, 1400, 1800, 2000}, false, false)
	verify(constant.BlocksPerEpoch, 10*constant.BlocksPerEpoch+1, []int64{600, 1000, 1400, 1800}, false, false)
	verify(constant.BlocksPerEpoch+1, constant.BlocksPerEpoch+1, []int64{}, false, false)
	verify(constant.BlocksPerEpoch+1, 10*constant.BlocksPerEpoch-1, []int64{600, 1000, 1400, 1800, 1999}, false, false)
	verify(constant.BlocksPerEpoch+1, 10*constant.BlocksPerEpoch, []int64{600, 1000, 1400, 1800, 2000}, false, false)
	verify(constant.BlocksPerEpoch+1, 10*constant.BlocksPerEpoch+1, []int64{600, 1000, 1400, 1800}, false, false)

}

func (ts *ProverTestSuite) TestError_setupHeadersForUpdate_neighboringEpoch() {
	...
	verify := func(latestHeight, nextHeight uint64) {
		...
		_, err = setupHeadersForUpdate(fn, headerFn, nil, clientStateLatestHeight, latestFinalizedHeader)
...
func (ts *ProverTestSuite) TestSuccess_getLatestFinalizedHeader() {

	verify := func(latestBlockNumber uint64) {
		getHeader := func(ctx context.Context, height uint64) (*types2.Header, error) {
...
func (ts *ProverTestSuite) TestError_getLatestFinalizedHeader_NoVote() {
