...
func setupHeadersForUpdate(
	...
	for epochHeight := firstUnsavedEpoch; epochHeight < latestFinalizedHeight; epochHeight += constant.BlocksPerEpoch {
		...
		if verifiableEpoch == nil {
			log.GetLogger().Warn("Insufficient vote attestation. Prover will stops.", "epoch", epochHeight, "trustedEpoch", trustedEpochHeight, "latest", latestHeight)
...
func setupNeighboringEpochHeader(
	...
	currentValidatorSet, currentTurnTerm, err := queryValidatorSetAndTurnTerm(getHeader, epochHeight)
	if err != nil {
		return nil, fmt.Errorf("setupNeighboringEpochHeader: failed to get current validator set: epochHeight=%d : %+v", epochHeight, err)
	}
	trustedValidatorSet, trustedTurnTerm, err := queryValidatorSetAndTurnTerm(getHeader, trustedEpochHeight)
	if err != nil {
	...
	if trustedValidatorSet.Contains(currentValidatorSet) {
		...
		nextCheckpoint := currentValidatorSet.Checkpoint(currentTurnTerm) + (epochHeight + constant.BlocksPerEpoch)
		...
		checkpoint := trustedValidatorSet.Checkpoint(trustedTurnTerm) + epochHeight
