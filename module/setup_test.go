...
func (ts *SetupTestSuite) TestSuccess_setupHeadersForUpdate_nonNeighboringEpoch() {
	...
	verify := func(latestHeight, nextHeight uint64, expectedEpochs []int64, nonNeighboringEpochFinalized bool, additionalNotFinalized bool) {
		...
		f := false
		notFinalized := &t
		queryVerifyingNeighboringEpochHeader := func(height uint64, _ uint64) (core.Header, error) {
			if *notFinalized {
				notFinalized = &f
				return nil, nil
			} else {
...
func (ts *SetupTestSuite) TestError_setupHeadersForUpdate_neighboringEpoch() {
	...
	verify := func(latestHeight, nextHeight uint64) {
		...
		_, err = setupHeadersForUpdate(fn, nil, headerFn, clientStateLatestHeight, latestFinalizedHeader, clienttypes.NewHeight(0, 1000000))
