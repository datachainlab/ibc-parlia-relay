...
func (ts *ProverNetworkTestSuite) TestSuccessCreateInitialLightClientState() {
	...
	cVal, cTurn, err := module.QueryValidatorSetAndTurnLength(ts.chain.Header, currentEpoch)
	ts.Require().NoError(err)
	pVal, pTurn, err := module.QueryValidatorSetAndTurnLength(ts.chain.Header, be.PreviousEpochBlockNumber(currentEpoch))
	ts.Require().NoError(err)
