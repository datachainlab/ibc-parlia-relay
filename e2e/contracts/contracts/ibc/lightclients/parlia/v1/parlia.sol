...
library IbcLightclientsParliaV1Fraction {
...
library IbcLightclientsParliaV1ClientState {
  ...
  function _read_trust_level(
    ...
    (IbcLightclientsParliaV1Fraction.Data memory x, uint256 sz) = _decode_IbcLightclientsParliaV1Fraction(p, bs);
    r.trust_level = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_IbcLightclientsParliaV1Fraction(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcLightclientsParliaV1Fraction.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcLightclientsParliaV1Fraction.Data memory r, ) = IbcLightclientsParliaV1Fraction._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += IbcLightclientsParliaV1Fraction._encode_nested(r.trust_level, pointer, bs);
...
library IbcLightclientsParliaV1ValidatorSet {


  //struct definition
  struct Data {
    IbcCoreClientV1Height.Data epoch_height;
    bytes[] validators;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_epoch_height(pointer, bs, r);
      } else
      if (fieldId == 2) {
    ...
    if (counters[2] > 0) {
      ...
      r.validators = new bytes[](counters[2]);
    }
    ...
    returns (uint)
  {
    ...
    pointer += IbcCoreClientV1Height._encode_nested(r.epoch_height, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcCoreClientV1Height._estimate(r.epoch_height));
    for(i = 0; i < r.validators.length; i++) {
   ...
   */
  function store(Data memory input, Data storage output) internal {
    IbcCoreClientV1Height.store(input.epoch_height, output.epoch_height);
...
library IbcLightclientsParliaV1Header {
  ...
  struct Data {
    ...
    bytes account_proof;
    IbcLightclientsParliaV1ValidatorSet.Data previous_validators;
    IbcLightclientsParliaV1ValidatorSet.Data current_validators;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_unpacked_repeated_headers(pointer, bs, nil(), counters);
      } else
      if (fieldId == 2) {
      ...
      if (fieldId == 4) {
        pointer += _read_previous_validators(pointer, bs, r);
      } else
      if (fieldId == 5) {
        pointer += _read_current_validators(pointer, bs, r);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_previous_validators(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (IbcLightclientsParliaV1ValidatorSet.Data memory x, uint256 sz) = _decode_IbcLightclientsParliaV1ValidatorSet(p, bs);
    r.previous_validators = x;
    return sz;
  }
   ...
   * @return The number of bytes decoded
   */
  function _read_current_validators(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (IbcLightclientsParliaV1ValidatorSet.Data memory x, uint256 sz) = _decode_IbcLightclientsParliaV1ValidatorSet(p, bs);
    r.current_validators = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_IbcLightclientsParliaV1ValidatorSet(uint256 p, bytes memory bs)
    internal
    pure
    returns (IbcLightclientsParliaV1ValidatorSet.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (IbcLightclientsParliaV1ValidatorSet.Data memory r, ) = IbcLightclientsParliaV1ValidatorSet._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.headers.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += IbcLightclientsParliaV1ETHHeader._encode_nested(r.headers[i], pointer, bs);
    }
    }
    ...
    pointer += IbcLightclientsParliaV1ValidatorSet._encode_nested(r.previous_validators, pointer, bs);
    ...
    pointer += IbcLightclientsParliaV1ValidatorSet._encode_nested(r.current_validators, pointer, bs);
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcLightclientsParliaV1ValidatorSet._estimate(r.previous_validators));
    e += 1 + ProtoBufRuntime._sz_lendelim(IbcLightclientsParliaV1ValidatorSet._estimate(r.current_validators));
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    IbcLightclientsParliaV1ValidatorSet.store(input.previous_validators, output.previous_validators);
    IbcLightclientsParliaV1ValidatorSet.store(input.current_validators, output.current_validators);
   ...
   * @param value The value to add
   */
  function addHeaders(Data memory self, IbcLightclientsParliaV1ETHHeader.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    IbcLightclientsParliaV1ETHHeader.Data[] memory tmp = new IbcLightclientsParliaV1ETHHeader.Data[](self.headers.length + 1);
