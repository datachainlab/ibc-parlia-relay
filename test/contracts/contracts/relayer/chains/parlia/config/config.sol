...
library RelayerProversParliaConfigProverConfig {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_trust_level_numerator(pointer, bs, r);
      } else
      if (fieldId == 2) {
        pointer += _read_trust_level_denominator(pointer, bs, r);
      } else
      if (fieldId == 3) {
   ...
   * @return The number of bytes decoded
   */
  function _read_trust_level_numerator(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (uint64 x, uint256 sz) = ProtoBufRuntime._decode_uint64(p, bs);
    r.trust_level_numerator = x;
    return sz;
  }
   ...
   * @return The number of bytes decoded
   */
  function _read_trust_level_denominator(
    uint256 p,
    bytes memory bs,
    Data memory r
  ) internal pure returns (uint) {
    (uint64 x, uint256 sz) = ProtoBufRuntime._decode_uint64(p, bs);
    r.trust_level_denominator = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_uint64(r.trust_level_numerator, pointer, bs);
    }
    if (r.trust_level_denominator != 0) {
    pointer += ProtoBufRuntime._encode_key(
      2,
      ProtoBufRuntime.WireType.Varint,
      pointer,
    ...
    pointer += ProtoBufRuntime._encode_uint64(r.trust_level_denominator, pointer, bs);
    }
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_uint64(r.trust_level_numerator);
    e += 1 + ProtoBufRuntime._sz_uint64(r.trust_level_denominator);
  ...
  function store(Data memory input, Data storage output) internal {
    output.trust_level_numerator = input.trust_level_numerator;
    output.trust_level_denominator = input.trust_level_denominator;
